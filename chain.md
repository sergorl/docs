...
2.      - В случае неупеха попытки добавления блок добавляется в пул Orphan'ов и переходим к шагу №3.
	- В случае успешного добавления блока осуществляестя попытка добавить в цепочку Orphan-блоки из пула до тех пор пока пул не пуст. Подобные блооки попадают на шаг №1.
3. Происходит уведомление всей системы об обновлениях при любом раскладе на предыдущем шаге.

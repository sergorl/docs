...
3. Проверяется пул Orphan'ов: если в пуле есть Orphan-блок-ребёнок обработанного блока, то осуществляестя попытка добавить в цепочку Orphan-блок-рёбнок. Подобные Orphan-блоки попадают на **шаг №1**. **Если в пуле нет блока-ребёнка, то обработка завершается.**
